...
def insert_before_once(path: str, marker: str, content: str) -> None:
    ...
    if marker not in text:
        raise SystemExit(f"Required marker was not found in {path}: {marker[:120]!r}")
...
# README: make bounded iteration regions the canonical product concept.
replace_once(
    "README.md",
...
replace_once(
    ...
    "M4 is in progress. It extends executable bounded iteration regions with multi-iteration execution, progress rules, independent-region outcome semantics, recovery hardening, and the complete Pi loop surface.",
)
replace_once(
    "README.md",
...
# Product specification: make loops generic and first-class.
replace_once(
...
# Roadmap: add independent region semantics to M4 and operational independence to M7.
replace_once(
...
# M4 plan: define generic loop semantics and add a vertical slice for independent regions.
replace_once(
...
# Hypagoal: autonomous continuation must use generic loop purposes.
replace_once(
...
# Trusted evaluation: make clear that evaluators support any iteration purpose.
replace_once(
...
# Bundled skill: generic authoring guidance, with failed checks as one special pattern.
replace_once(

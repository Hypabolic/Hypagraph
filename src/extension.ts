...
export default function hypagraphExtension(pi: ExtensionAPI): void {
  ...
  pi.on("before_agent_start", async (event) => {
    ...
    const active = activeNode(state);
    return {
      systemPrompt: `${event.systemPrompt}\n\nHYPAGRAPH CONTROL:\n${renderWorkflow(state)}\nUse hypagraph_transition before and after work. Work only on the active node. Give concrete evidence before completion. Ready nodes are [${ready.join(", ")}].${active ? ` The active node is '${active.id}'.` : " Start one ready node before you change the repository."}`,
    };
  });
  ...
  pi.registerTool({
    ...
    description: "Define and validate a directed coding workflow.",
    promptSnippet: "Define a validated workflow before multi-step coding work",
    promptGuidelines: ["Use hypagraph_define for work that needs explicit dependencies and evidence."],
  ...
  pi.registerTool({
    ...
    description: "Read the workflow, active node, ready nodes, attempts, and node states.",
  ...
  pi.registerTool({
    ...
    description: "Start, complete, block, or unblock one node through the event-driven lifecycle.",
    promptSnippet: "Move a Hypagraph node through its lifecycle",
    promptGuidelines: ["Start before work. Complete with concrete evidence after work."],
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      if (params.action === "start") {
        run({ type: "start-node", nodeId: params.nodeId, attemptId: randomUUID(), commandId: randomUUID(), correlationId, at });
      } else if (params.action === "complete") {
        const attemptId = state.runtime.nodes[params.nodeId]?.currentAttemptId;
        if (!attemptId) throw new Error(`Node '${params.nodeId}' has no current attempt.`);
        run({ type: "submit-result", nodeId: params.nodeId, attemptId, evidence: params.evidence ?? [], commandId: randomUUID(), correlationId, at });
        run({ type: "begin-verification", nodeId: params.nodeId, attemptId, commandId: randomUUID(), correlationId, at });
        run({ type: "complete-verification", nodeId: params.nodeId, attemptId, passed: true, commandId: randomUUID(), correlationId, at });
      } else if (params.action === "block") {
        run({ type: "block-node", nodeId: params.nodeId, reason: params.reason ?? "", commandId: randomUUID(), correlationId, at });
      } else {
        run({ type: "unblock-node", nodeId: params.nodeId, commandId: randomUUID(), correlationId, at });
      }
      updateUi(state, ctx);
      return textResult(renderWorkflow(state));
    },

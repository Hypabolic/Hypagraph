...
export type ReadyCodeDecision = GoalContinuationStateIdentity & {
  kind: "run-ready-code";
  nodeId: string;
  loopId?: string;
};

export function isReadyCodeDecision(decision: GoalContinuationDecision): decision is ReadyCodeDecision {
  return decision.kind === "run-ready-code";
}

export interface DeterministicCodeDispatchRequest {
  dispatchId: string;
  decision: ReadyCodeDecision;
  at: string;
}

export type DeterministicCodeDispatchResult =
  | { ok: true; state: HypagraphState; events: DomainEvent[] }
  | { ok: false; diagnostics: Diagnostic[] };

const reject = (code: string, message: string, location?: string): DeterministicCodeDispatchResult => ({
  ok: false,
...
const validateSelection = (
  state: HypagraphState,
  request: DeterministicCodeDispatchRequest,
): DeterministicCodeDispatchResult | undefined => {
  ...
  if (goal.status !== "active" || state.phase !== "running") {
    return reject("goal_not_active", `The goal cannot dispatch a deterministic code node while it is '${goal.status}'.`);
  }
  ...
  if (!request.dispatchId.trim()) {
    return reject("action_dispatch_id_required", "A deterministic code dispatch requires an ID.", "dispatchId");
  }
  if (!Number.isFinite(Date.parse(request.at))) {
    return reject("action_dispatch_timestamp_invalid", "A deterministic code dispatch requires a valid timestamp.", "at");
  }

  const decision = request.decision;
  if (decision.goalId !== goal.goalId || decision.workflowId !== state.workflowId) {
    return reject("stale_action_dispatch", "The deterministic code selection belongs to a different goal or workflow.");
  }
  if (decision.revision !== state.revision || decision.sequence !== state.sequence || decision.snapshotHash !== state.snapshotHash) {
    return reject("stale_action_dispatch", "Canonical state changed before the deterministic code node was dispatched.");
  }
  if (decision.continuationOrdinal !== goal.continuationOrdinal) {
    return reject("stale_action_dispatch", "The scheduler ordinal changed before the deterministic code node was dispatched.");
  }

  const selected = selectGoalContinuation(state);
  if (!isDispatchableGoalContinuation(selected)
    || selected.kind !== "run-ready-code"
    || !continuationActionMatches(selected, decision)) {
    return reject("stale_action_dispatch", "The selected deterministic code node changed before dispatch.", "decision");
  }
  return undefined;
};

export function beginReadyCodeDispatch(
  state: HypagraphState,
  request: DeterministicCodeDispatchRequest,
): DeterministicCodeDispatchResult {
...
export function finishReadyCodeDispatch(
  state: HypagraphState,
  request: DeterministicCodeDispatchRequest,

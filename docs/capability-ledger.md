...
| Named recipe library (beyond one implement-verify tool) | Yes | Yes | Partial | No | Two construction recipes: implement-verify loop (`hypagraph_recipe_implement_verify_loop`) and flagship implement/parallel-review (`hypagraph_recipe_implement_parallel_review`: implement → review-panel multi-child fan-out → ordinary join → integrate). Child reviewer templates and create-child runtime steps are skill- and tool-documented. Launch-with-args catalog and Live dogfood of the flagship multi-agent path remain open. |

...
| Aggregate / quorum / synthesis nodes | Yes | Yes | Yes | No | Ordinary multi-child all-success join is on the product path. Domain: `child-outcome-synthesis.ts` (schema v1 all-success policy/result; pure evaluate; host-default or declared `join.passed`). Host: `family-product-synthesis.ts`; extension applies ready join after child returns and on restore re-entry. Wait set: parent may create siblings while `waiting_for_child`; parent stays waiting while any sibling for that parent node is active; auto join runs when every sibling is terminal. Host publishes default fact `join.passed` without author produce. Ordinary path does not require hand `expectedBindingCount` (multi-child wait set; AUTO_JOIN_MIN=2). Pass leaves parent running for integration. Fail publishes `join.passed`=false and blocks parent unless child failure policy already owns the parent (quiet-skip). One-child does not auto multi-join under AUTO_JOIN_MIN=2. Optional advanced: declare boolean produce `join.passed` for gates; optional `expectedBindingCount`. Full quorum, ranked, and model synthesis strategies, named multi-agent recipes, and aggregate node kinds are not shipped. Live dogfood still open (no case ID). |
